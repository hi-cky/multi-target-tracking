...
# 过程级别中文注释：定义默认 Qt 版本，安装路径稍后根据 Homebrew 自动探测
QT_VERSION="${QT_VERSION:-6.9.0}"

# 过程级别中文注释：确保 Xcode Command Line Tools 就绪
...
# 过程级别中文注释：检查 Homebrew，并提示通过官方网站安装
if ! command -v brew >/dev/null 2>&1; then
    echo "[缺失] 未检测到 Homebrew，请参考 https://brew.sh 安装"
    exit 1
fi

# 过程级别中文注释：安装 CMake 与 Ninja 以配合 Zed 构建
brew install cmake ninja || true

# 过程级别中文注释：安装 Qt CLI 版本，供 CMakePresets 使用
brew install qt@6 || true

# 过程级别中文注释：根据 brew --prefix 自动探测 Qt 安装路径，允许用户通过 QT_PREFIX 覆盖
...
# 过程级别中文注释：如果 ~/.zprofile 尚未包含当前路径，则追加一行，避免重复
